...
function createDescription(rule) {
    ...
    const description = core.getInput('displayAnnotationDescription', { required: false }) === 'true' ? lines.join('\n') + '\n\n' : '';
    // const link = `<a href="${rule.helpUri.trim()}" type="text/html">${rule.id}</a>`
    const link = 'https://github.com/lpohl-Reply/pmd-github-action/wiki';
const desc =
`${description}${link} (Priority: ${rule.properties.priority}, Ruleset: ${rule.properties.ruleset})`;
    return desc;
}
