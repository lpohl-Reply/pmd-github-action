...
function trimPathToSubstring(absPath, subString) {
    absPath = String(absPath);
    subString = String(subString);
    var index = absPath.indexOf(subString);
    core.debug(`index: ${index}`);
    core.debug(`subString: ${subString}`);
    core.debug(`absPath: ${absPath}`);

    if (index !== -1) {
        return absPath.substring(index);
    } else {
        return absPath; // If substring not found, return the original absolute path
    }
}
...
const processSarifReport = function (report) {
    ...
    results.forEach(violation => {
        ...
        violation.locations.forEach(location => {
            ...
            core.info(`\n${annotation.sourcePath}::::${annotation.file}:${annotation.startLine}:${rule.id} (Priority: ${rule.properties.priority}):${violation.message.text}`);
...
function createAnnotation(location, title) {
    const sourcePath = core.getInput('sourcePath', { required: false });
    const uri = trimPathToSubstring(location.artifactLocation.uri, sourcePath);
    return {
        title: title,
        file: uri,
...
function createDescription(rule) {
    ...
    return helper.generate_description(lines, rule);
}
